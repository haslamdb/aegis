...
RENAL_ADJUSTMENTS = {
    ...
    "meropenem": {
        ...
        "hd": {"dose_mg_kg": 10, "interval_h": 24, "note": "Dose after HD session"},
        "severity": "high",
    ...
    "cefepime": {
        ...
        "hd": {"dose_mg": 1000, "interval_h": 24, "note": "Dose after HD. Risk of neurotoxicity if not adjusted."},
    ...
    "ceftazidime": {
        ...
        "hd": {"dose_mg": 1000, "frequency": "after_each_hd", "note": "1g after each HD session"},
        "severity": "moderate",
        "source": "Sanford Guide 2024",
    },
    "acyclovir": {
        ...
        "hd": {"interval_h": 24, "frequency": "after_hd", "note": "Dose after HD. Risk of nephrotoxicity and neurotoxicity."},
        "note": "Nephrotoxicity risk with high doses or inadequate hydration",
        "severity": "high",
        "source": "Sanford Guide 2024",
    },
    "fluconazole": {
        "gfr_lt_50": {"dose_reduction": 0.5, "note": "50% dose reduction for CrCl < 50 (after loading dose)"},
        "hd": {"dose_reduction": 0.5, "frequency": "after_hd", "note": "50% dose after each HD"},
        "severity": "moderate",
        "source": "Sanford Guide 2024",
    },
    "piperacillin_tazobactam": {
        ...
        "hd": {"dose_mg": 2250, "interval_h": 6, "note": "Extra 750 mg dose after each HD"},
        "crrt": {"dose_mg": 4500, "interval_h": 6, "note": "Full dose on CRRT with extended infusion"},
        "severity": "moderate",
        "source": "Sanford Guide 2024",
    },
    "ciprofloxacin": {
        "normal": {"dose_mg": 400, "interval_h": 12},
        "gfr_lt_30": {"dose_reduction": 0.5, "interval_h": 12, "note": "50% dose reduction for CrCl < 30"},
        "hd": {"dose_mg": 400, "interval_h": 24, "note": "Dose after HD"},
        "severity": "moderate",
        "source": "Sanford Guide 2024",
    },
    "levofloxacin": {
        "normal": {"dose_mg": 750, "interval_h": 24},
        "gfr_20_49": {"dose_mg": 750, "then": {"dose_mg": 500, "interval_h": 24}, "note": "Initial dose 750 mg, then 500 mg q24h"},
        "gfr_10_19": {"dose_mg": 750, "then": {"dose_mg": 500, "interval_h": 48}, "note": "Initial dose 750 mg, then 500 mg q48h"},
        "gfr_lt_10": {"dose_mg": 750, "then": {"dose_mg": 250, "interval_h": 48}, "note": "Initial dose 750 mg, then 250 mg q48h"},
        "hd": {"dose_mg": 750, "then": {"dose_mg": 500, "interval_h": 48}, "note": "Initial 750 mg, then 500 mg q48h. No supplemental dose after HD."},
        "severity": "moderate",
        "source": "Sanford Guide 2024",
    },
    "gentamicin": {
        ...
        "extended_interval_cutoffs": {
            ...
            "gfr_lt_20": {"note": "Traditional dosing (1-2 mg/kg q8-12h) with close level monitoring"},
        },
        "hd": {"dose_mg_kg": 1.5, "frequency": "after_hd", "note": "1.5 mg/kg after each HD, check levels"},
        "crrt": {"dose_mg_kg": 2.5, "interval_h": 24, "note": "Loading dose 2.5 mg/kg, then adjust based on levels"},
        "severity": "high",
    ...
    "amikacin": {
        ...
        "hd": {"dose_mg_kg": 5, "frequency": "after_hd", "note": "5 mg/kg after each HD, check levels"},
        "severity": "high",
        "source": "Sanford Guide 2024",
    },
    "trimethoprim_sulfamethoxazole": {
        "normal": {"dose_tmp_mg_kg": 5, "interval_h": 6},
        "gfr_15_30": {"dose_reduction": 0.5, "note": "50% dose reduction"},
        "gfr_lt_15": {"contraindicated": True, "note": "Avoid use. Risk of hyperkalemia and bone marrow suppression."},
        "severity": "high",
        "source": "Sanford Guide 2024",
    },
    "cefazolin": {
        ...
        "hd": {"dose_mg": 1000, "frequency": "after_hd", "note": "1g after each HD"},
        "severity": "moderate",
        "source": "Sanford Guide 2024",
    },
    "nafcillin": {
...
class RenalAdjustmentRules(BaseRuleModule):
    ...
    def evaluate(self, context: PatientContext) -> list[DoseFlag]:
        ...
        for med in context.antimicrobials:
            ...
            if adjustment.get("method") in ("level_based", "auc_based"):
                if adjustment.get("flag_if_no_level"):
                    pass  # TODO: Check if drug levels are ordered
    ...
    def _check_gfr_adjustment(self, med, adjustment, gfr, context, drug_key):
        tier = None
        ...
        if expected.get("contraindicated"):
            return DoseFlag(
                ...
                details={"gfr": gfr, "tier": tier, "note": expected.get("note", "")},
            )
    ...
    def _check_dialysis_dosing(self, med, dialysis_rule, context, drug_key):
        adjustment = RENAL_ADJUSTMENTS[drug_key]
    ...
    def _find_tier(self, adjustment, possible_keys):
