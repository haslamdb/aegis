...
class CCHMCGuidelinesEngine:
    ...
    def _check_agent_against_disease(
        self,
        disease: dict,
        ...
        """Check prescribed agent appropriateness against a disease entry.

        Shared logic used by both check_agent_appropriateness and
        check_agent_by_disease_ids.

        Args:
            disease: Disease dictionary from guidelines JSON
    ...
    def check_agent_appropriateness(
        ...
        return self._check_agent_against_disease(
            disease=disease,
            prescribed_agent=prescribed_agent,
            patient_age_months=patient_age_months,
            allergies=allergies,
            confidence=best_match["confidence"],
        )

    def check_agent_by_disease_ids(
        self,
        ...
        """Check agent appropriateness using guideline disease IDs directly.

        This is the primary method for the Django module - takes disease_ids
        from taxonomy extraction (not ICD-10 codes).

        Args:
            disease_ids: List of guideline disease IDs from taxonomy
            prescribed_agent: The antibiotic that was prescribed
            patient_age_months: Patient age in months
            allergies: List of known allergies
        ...
        Returns:
            AgentRecommendation with appropriateness assessment
        """
        result = AgentRecommendation(current_agent=prescribed_agent)

        if not disease_ids:
            result.recommendation = "No guideline disease IDs provided"
            return result

        # Find matching disease by disease_id
        best_disease = None
        body_systems = self.disease_guidelines.get("body_systems", {})
        for system_id, system_data in body_systems.items():
            for disease in system_data.get("diseases", []):
                if disease.get("disease_id") in disease_ids:
        ...
        if not best_disease:
            result.recommendation = f"No CCHMC guideline found for disease IDs: {disease_ids}"
            return result
