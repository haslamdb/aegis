...
# Note type LOINC codes for targeted FHIR queries
NOTE_TYPES = {
...
# Keywords indicating antibiotic reassessment/review at 48-72h
...
class NoteChecker(ElementChecker):
    """Check note-based bundle elements (documentation requirements).

    Handles reassessment documentation, cellulitis margin marking,
    risk stratification, and generic documentation checks.
    """
    ...
    def check(
        ...
        **kwargs,
        ...
        Routes to specific sub-checker based on element_id pattern.
    ...
    def _check_reassessment(
        ...
        Per ASP guidelines, antibiotic therapy should be reassessed at
        48-72 hours with culture data and clinical response.
        """
        ...
        # If we are not yet at 48h, element is pending
        ...
        # Get notes from the reassessment window
        note_loinc = NOTE_TYPES.get('id_consult', []) + NOTE_TYPES.get('progress_note', [])
        notes = self.fhir_client.get_recent_notes(
            patient_id=patient_id,
            since_hours=72,
            note_types=note_loinc,
        )
        ...
        if now < reassess_end:
            return self._create_result(
                ...
                trigger_time=trigger_time,
                notes=(
                    f"In reassessment window (48-72h) - awaiting documentation. "
                    f"Window closes: {reassess_end.strftime('%Y-%m-%d %H:%M')}"
                ),
            )
        else:
            return self._create_result(
    ...
    def _check_margins_marked(
        ...
        For SSTI/cellulitis bundle, borders should be outlined with
        pen to track progression vs resolution.
        """
        notes = self.fhir_client.get_recent_notes(
            patient_id=patient_id,
            since_hours=24,
    ...
    def _check_risk_stratification(
        ...
        """Check for risk stratification documentation.

        Used by febrile neutropenia bundle and others requiring
    ...
    def _check_generic_documentation(
        ...
        Extracts keywords from element description and searches
        recent clinical notes for matches.
        """
    ...
    def _extract_keywords(self, text: str) -> list[str]:
        ...
        # Also look for multi-word clinical phrases
        phrases = [
            'blood culture', 'id consult', 'infectious disease',
