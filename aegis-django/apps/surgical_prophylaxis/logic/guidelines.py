"""
Surgical prophylaxis guidelines configuration.

Loads CCHMC guidelines JSON and provides lookup methods for
procedure requirements, dosing info, redose intervals, etc.
"""
...
from apps.surgical_prophylaxis.models import ProcedureCategory
...
# CPT prefix -> ProcedureCategory mapping
CPT_CATEGORY_HINTS: dict[str, str] = {
    '336': ProcedureCategory.CARDIAC,
    '337': ProcedureCategory.CARDIAC,
    '338': ProcedureCategory.CARDIAC,
    '339': ProcedureCategory.CARDIAC,
    '324': ProcedureCategory.THORACIC,
...
class ProcedureRequirement:
    ...
    requires_postop_continuation: bool = False
    postop_continuation_allowed: bool = False
    postop_duration_hours: Optional[int] = None
    postop_interval_hours: Optional[float] = None
...
DEFAULT_DOSING: dict[str, DosingInfo] = {
    'cefazolin': DosingInfo(
        ...
        pediatric_mg_per_kg=40.0, pediatric_max_mg=2000.0,
        adult_standard_mg=2000.0, adult_high_weight_threshold_kg=100.0,
        adult_high_weight_mg=3000.0, redose_interval_hours=3.0,
    ),
    'cefoxitin': DosingInfo(
        medication_name='cefoxitin',
        pediatric_mg_per_kg=40.0, pediatric_max_mg=2000.0,
        adult_standard_mg=2000.0, redose_interval_hours=3.0,
    ),
    'ceftriaxone': DosingInfo(
        medication_name='ceftriaxone',
        pediatric_mg_per_kg=50.0, pediatric_max_mg=2000.0,
        adult_standard_mg=2000.0, redose_interval_hours=12.0,
    ),
    ...
    'vancomycin': DosingInfo(
        ...
        pediatric_mg_per_kg=15.0, pediatric_max_mg=2000.0,
        adult_standard_mg=2000.0, redose_interval_hours=8.0,
        infusion_time_minutes=60,
    ),
    'metronidazole': DosingInfo(
        medication_name='metronidazole',
        pediatric_mg_per_kg=15.0, pediatric_max_mg=1000.0,
        adult_standard_mg=1000.0, redose_interval_hours=12.0,
    ),
    'clindamycin': DosingInfo(
        medication_name='clindamycin',
        pediatric_mg_per_kg=10.0, pediatric_max_mg=900.0,
        adult_standard_mg=900.0, redose_interval_hours=6.0,
    ),
    'gentamicin': DosingInfo(
        medication_name='gentamicin',
        pediatric_mg_per_kg=4.5, pediatric_max_mg=160.0,
        adult_standard_mg=360.0, redose_interval_hours=12.0,
    ),
    'ampicillin': DosingInfo(
        medication_name='ampicillin',
        pediatric_mg_per_kg=50.0, pediatric_max_mg=2000.0,
        adult_standard_mg=2000.0, redose_interval_hours=2.0,
    ),
    'ampicillin-sulbactam': DosingInfo(
        medication_name='ampicillin-sulbactam',
        pediatric_mg_per_kg=75.0, pediatric_max_mg=3000.0,
        adult_standard_mg=3000.0, redose_interval_hours=2.0,
    ),
    'piperacillin-tazobactam': DosingInfo(
        medication_name='piperacillin-tazobactam',
        pediatric_mg_per_kg=100.0, pediatric_max_mg=3375.0,
        adult_standard_mg=3375.0, redose_interval_hours=2.0,
    ),
    'aztreonam': DosingInfo(
        medication_name='aztreonam',
        pediatric_mg_per_kg=30.0, pediatric_max_mg=2000.0,
        adult_standard_mg=2000.0, redose_interval_hours=4.0,
    ),
    'ciprofloxacin': DosingInfo(
        medication_name='ciprofloxacin',
        pediatric_mg_per_kg=10.0, pediatric_max_mg=400.0,
        adult_standard_mg=400.0, redose_interval_hours=8.0,
    ),
}
...
class GuidelinesConfig:
    ...
    def get_duration_limit(self, procedure_category: str) -> int:
    ...
    def get_timing_window(self, medication_name: str) -> int:
        ...
        if any(ext in med_lower for ext in Config.EXTENDED_WINDOW_ANTIBIOTICS):
            return Config.EXTENDED_TIMING_WINDOW
        return Config.STANDARD_TIMING_WINDOW
...
def get_guidelines_config() -> GuidelinesConfig:
    """Get or create the global guidelines configuration instance."""
    global _config
    if _config is None:
