...
Parses ADT (patient tracking) and ORM/SIU (scheduling) messages
for real-time surgical prophylaxis monitoring.

Adapted from surgical-prophylaxis/src/realtime/hl7_parser.py (428 lines).
...
class HL7Message:
    ...
    def patient_name(self) -> str:
        ...
        if len(name_components) >= 2:
            return f"{name_components[1]} {name_components[0]}"
...
def extract_orm_o01_data(message: HL7Message) -> dict:
    ...
    for i, orc in enumerate(orc_segments):
        ...
        if i < len(obr_segments):
            ...
            order["scheduled_datetime"] = parse_hl7_datetime(obr.get_field(36)) or order["scheduled_datetime"]
...
def build_ack_message(original_message: HL7Message, ack_code: str = "AA", error_message: str = "") -> str:
