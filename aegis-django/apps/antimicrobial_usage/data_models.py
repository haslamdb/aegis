"""Data models for Antimicrobial Usage Alerts.

Dataclasses for broad-spectrum antibiotic duration monitoring.
Indication monitoring (ICD-10 + LLM) is deferred to a separate module.
"""

from dataclasses import dataclass, field
from datetime import datetime

from django.utils import timezone


@dataclass
class Patient:
    """Patient information from FHIR."""
    fhir_id: str
    mrn: str
    name: str
    birth_date: str | None = None
    gender: str | None = None
    location: str | None = None
    department: str | None = None


@dataclass
class MedicationOrder:
    """Active medication order from FHIR."""
    fhir_id: str
    patient_id: str
    medication_name: str
    rxnorm_code: str | None = None
    dose: str | None = None
    route: str | None = None
    start_date: datetime | None = None
    status: str = "active"

    @property
    def duration_hours(self) -> float | None:
        """Calculate hours since medication started."""
        if self.start_date is None:
            return None
        now = timezone.now()
        # Ensure start_date is timezone-aware
        start = self.start_date
        if start.tzinfo is None:
            start = timezone.make_aware(start)
        delta = now - start
        return delta.total_seconds() / 3600

    @property
    def duration_days(self) -> float | None:
        """Calculate days since medication started."""
        hours = self.duration_hours
        return hours / 24 if hours else None


@dataclass
class UsageAssessment:
    """Assessment of broad-spectrum antibiotic usage duration."""
    patient: Patient
    medication: MedicationOrder
    duration_hours: float
    threshold_hours: float
    exceeds_threshold: bool
    recommendation: str
    assessed_at: datetime = field(default_factory=timezone.now)
    severity: str = 'high'  # Django AlertSeverity value

    # Optional context
    related_cultures: list[str] = field(default_factory=list)
    justification_found: bool = False
    justification_reason: str | None = None
